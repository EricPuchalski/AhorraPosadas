...
import com.ahorraposadas.scrapermsc.config.HiperLibertadScraperProperties;
...
public class HiperLibertadScraperService implements ScraperService {
    ...
    public HiperLibertadScraperService(ProductRepository productRepository,
                                       KafkaTemplate<String, PriceChangeEvent> kafkaTemplate,
                                       HiperLibertadScraperProperties properties) {
    ...
    public void scrapeProducts() {
        ...
        for (String categoryUrl : categories) {
            logger.info("Scrapeando categoria: {}", categoryUrl);
            try {
                Document document = Jsoup.connect(categoryUrl)
                ...
                logger.error("Error en URL {}", categoryUrl, e);
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
                logger.warn("Scraping interrumpido en URL {}", categoryUrl, e);
                break;
            }
        }
    ...
    private String processProductElement(Element productElement) {
        try {
            ...
            Element priceContainer = productElement.selectFirst(properties.getPriceContainerSelector());

            if (titleElement == null || linkElement == null || imageElement == null || priceContainer == null) {
            ...
            Element priceSpan = priceContainer.selectFirst(properties.getPriceSpanSelector());
            if (priceSpan == null) {
                logger.warn("Precio no encontrado para producto: {}", title);
                return null;
            }

            BigDecimal price = parsePrice(priceSpan.text());
